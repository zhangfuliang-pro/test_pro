...
#ifdef CONFIG_VERSION_FPGA
    gd->xtal_clk=12000000;
#else
	gd->xtal_clk=12000000;
#endif
...
int board_late_init(void)
{
    ...
    printf("*******************************************\nboard_late_init for status check\n");
    ...
    if(u16Status)
    {
        ...
        if(u16Status & KEY_CUST_BIT)
        {
            ...
            CLRREG16(REG_ADDR_STATUS, KEY_CUST_BIT);
        }
        if(u16Status & UBOOT_BIT)
        {
            ...
            printf("%s\n", buffer);
            run_command(buffer, 0);
            CLRREG16(REG_ADDR_STATUS, UBOOT_BIT);
        }
        if(u16Status & ZBOOT_BIT)
        {
            ...
            printf("%s\n", buffer);
            run_command(buffer, 0);
            CLRREG16(REG_ADDR_STATUS, ZBOOT_BIT);
        }
        if(u16Status & KERNEL_BIT)
        {
            ...
            printf("%s\n", buffer);
            run_command(buffer, 0);
            CLRREG16(REG_ADDR_STATUS, KERNEL_BIT);
        }
        if(u16Status & FORCE_UBOOT_BIT)
        {
            CLRREG16(REG_ADDR_STATUS, FORCE_UBOOT_BIT);
            printf("\nFORCE UBOOT BY STATUS REGISTER\n\n");
            return 0;
        }
