...
void MHal_GPIO_Pad_Set(MS_GPIO_NUM u32IndexGPIO)
{
    ...
    case PAD_FUART_TX:
        ...
        if((MHal_CHIPTOP_REG(REG_FUART_MODE)&0x3)==0x1) //reg_fuart_mode[1:0]=0x1
            MHal_CHIPTOP_WriteRegBit(REG_FUART_MODE, DISABLE, 0x3);  //bit[1:0]=0
        if((MHal_CHIPTOP_REG(REG_UART0_MODE)&0x30)==0x20) //reg_uart0_mode[5:4]=0x2
            MHal_CHIPTOP_WriteRegBit(REG_UART0_MODE, DISABLE, 0x30);  //bit[5:4]=0
        break;
    case PAD_FUART_CTS:
    case PAD_FUART_RTS:
        MHal_CHIPTOP_WriteRegBit(REG_TEST_IN_MODE, DISABLE, BIT1|BIT0);
        ...
        if((MHal_CHIPTOP_REG(REG_FUART_MODE)&0x3)==0x1) //reg_fuart_mode[1:0]=0x1
            MHal_CHIPTOP_WriteRegBit(REG_FUART_MODE, DISABLE, 0x3);  //bit[1:0]=0
        if((MHal_CHIPTOP_REG(REG_UART1_MODE)&0x3)==0x2) //reg_uart1_mode[1:0]=0x2
            MHal_CHIPTOP_WriteRegBit(REG_UART1_MODE, DISABLE, 0x3);  //bit[1:0]=0
        break;
