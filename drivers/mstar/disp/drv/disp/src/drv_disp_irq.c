...
#if defined(DISP_OS_TYPE_LINUX_KERNEL) && defined(DISP_INTERNAL_ISR)
static irqreturn_t DrvDispIrqInternalIsrCb(int eIntNum, void* pstDevParam)
{
    u32 u32Flag;
...
bool DrvDispIrqCreateInternalIsr(void)
{
    ...
    if(bSupported)
    {
        ...
        u32IrqNum = gstDispIrqCfg[u8DeviceIdx].u32IrqNum;
         ...
         s32IrqRet = request_irq(u32IrqNum, DrvDispIrqInternalIsrCb, IRQF_TRIGGER_NONE, "mdisp_interisr", NULL);
        if(s32IrqRet)
        {
