...
void Hal_CARD_IPOnceSetting(IPEmType eIP)
{
    ...
    if(eIP == EV_IP_FCIE1)
    {
        ...
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK,REG_CLK_SD0), BIT05_T); //[5:2]: Clk_Sel [1]: Clk_i [0]: Clk_g
#else
...
void Hal_CARD_SetPADToPortPath(IPEmType eIP, PortEmType ePort, PADEmType ePAD, BOOL_T bTwoCard)
{
    ...
    if(eIP == EV_IP_FCIE1)
    {
        if(ePAD == EV_PAD1)  //PAD_SD
        {
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CHIPTOP_BANK, 0x08), BIT02_T | BIT03_T | BIT08_T);         //sd mode2 = 0, sdio mode = 0
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CHIPTOP_BANK, 0x08), BIT02_T);         //sd mode = 01
