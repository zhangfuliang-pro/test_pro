...
#define A_MCM_SC_GP_BANK    GET_CARD_REG_ADDR(A_RIU_BASE, 0x89980)
...
void Hal_CARD_IPOnceSetting(IPEmType eIP)
{
    ...
    else if(eIP == EV_IP_FCIE2)
    {
        ...
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_MCM_SC_GP_BANK,0x25), BIT07_T); //[7]: select BOOT clock source (glitch free) - 0: select BOOT clock 12MHz (xtali), 1: select FCIE/SPI clock source
    }
