...
//    #define SER_FLASH_TIME(_stamp)           (get_ms_ticks(&_stamp))
...
{
    MS_U32 u32NsTime = 0;
	MS_U32 t1=get_ms_ticks();
	u32NsTime=((tPreTime>t1)?((get_full_ms_ticks()-tPreTime)+t1):(t1-tPreTime));
...
#if defined (MSOS_TYPE_LINUX)
    } while (!_Hal_GetMsTime(time_st, SERFLASH_SAFETY_FACTOR * 30));
#else
...
MS_BOOL HAL_SERFLASH_BlockErase(MS_U32 u32StartBlock, MS_U32 u32EndBlock, MS_BOOL bWait)
{
    ...
    for( u32I = u32StartBlock; u32I <= u32EndBlock; u32I++)
    {
        ...
        if(bRet) printf(" ## erasing 0x%08X success...\n",(unsigned int)u32FlashAddr);
        else printf("  ## erasing 0x%08X failed!!\n",(unsigned int)u32FlashAddr);
    }
...
MS_BOOL HAL_SERFLASH_DMA(MS_U32 u32FlashStart, MS_U32 u32DRASstart, MS_U32 u32Size)
{
    ...
    MS_U32 u32Timeout =((SERFLASH_SAFETY_FACTOR*u32Size) >> 11);
#else
    MS_U32 u32Timer;
...
#if defined (MSOS_TYPE_LINUX)
    } while (!_Hal_GetMsTime(time_st, u32Timeout));
#else
    } while (!SER_FLASH_EXPIRE(u32Timer));
#endif
...
static void _HAL_FSP_WaitForDone(void)
{
    ...
    MS_U32 start_time = get_ms_ticks();
    //MS_U32 u32Timer;
...
#define DEFAULT_FSP_TIMEOUT	2000
	...
	while(1)//Polling Done
	{
		...
		if( _Hal_GetMsTime(start_time, DEFAULT_FSP_TIMEOUT ) )
		{
