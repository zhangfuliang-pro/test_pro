...
void msb250x_udc_init_hw(void)
{
    ...
    ms_writew(0x6BC3, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x00)); // Turn on UPLL, reg_pdn: bit<9> reg_pdn: bit<15>, bit <2> ref_pdn
    mdelay(1);
...
void msb250x_udc_init_otg(struct usb_gadget_driver *driver)
{
    //u8 ep_num = 0;
    u8 power = 0;
    u8 usb_cfg_l = 0;
    ...
    struct otg0_usb_cfg0_l* pst_otg0_usb_cfg0_l = (struct otg0_usb_cfg0_l*) &usb_cfg_l;
    ...
    pst_otg0_usb_cfg0_l->bSRST_N = 0;                //low active
    ms_writeb(usb_cfg_l, MSB250X_OTG0_USB_CFG0_L);      // Reset OTG
    pst_otg0_usb_cfg0_l->bSRST_N = 1;                //default set as 1
    ...
    */
    ...
    if (USB_SPEED_HIGH == driver->speed)
    {
        pst_power->bHSEnab = 1;
    }
    else
