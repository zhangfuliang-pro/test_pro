...
//////////////////////////////////////////////////////////////////////
//add kjf for 2020/01/03
#define SENSOR_NUM_MAX 4
typedef struct __Sensor_param
{
	char *sensor_id;
	char *dev_num;
	char *chip_addr;
    ...
    int Pdwn;
    int Rst;
}T_Sensor_param,*PT_Sensor_param_T;

T_Sensor_param sensor_i2c_param[] =
{
	{
		.sensor_id = "gc2053",
		.dev_num = "1",
...
char g_sensor_id[24] = {0};

static int check_sensor(void)
{
	//printf(">>>>start check sensor\n");
	int i = 0;
	int ret = -1;
	char get_sensor_id[24] = {0};
	...
	do{
		...
		Set_SNR_PdnPad(1);
		Set_SNR_RstPad(0);
		Set_SNR_MCLK(0, sensor_i2c_param[i].MclkIdx>=999?0:1, sensor_i2c_param[i].MclkIdx);

		SNR_PowerDown(0, sensor_i2c_param[i].Pdwn);
		SNR_Reset(0, 0);
		udelay(1000*1000);
		//enable sensor
		Set_SNR_PdnPad(sensor_i2c_param[i].PdnPad);
		Set_SNR_RstPad(sensor_i2c_param[i].RstPad);
		Set_SNR_MCLK(0, sensor_i2c_param[i].MclkIdx>=999?0:1, sensor_i2c_param[i].MclkIdx);

		SNR_PowerDown(0, sensor_i2c_param[i].Pdwn);
		SNR_Reset(0, sensor_i2c_param[i].Rst);

		// printf("\n Sensor configuration, PdnPad=%u, RstPad=%u, MCLK=%u, PWD_PIN=%u, RST_PIN=%u \n\n",
		// 		sensor_i2c_param[i].PdnPad, sensor_i2c_param[i].RstPad, 
		// 		sensor_i2c_param[i].MclkIdx, sensor_i2c_param[i].Pdwn, 
		// 		sensor_i2c_param[i].Rst);

		//step 2:start sensor i2c dev
		set_i2c_dev(sensor_i2c_param[i].dev_num);
		//step 3:check sensor for i2c
		// printf(">>>>[%s][%s][%s][%s]\n",sensor_i2c_param[i].dev_num,sensor_i2c_param[i].chip_addr,
		// 			sensor_i2c_param[i].data_addr,sensor_i2c_param[i].data_len);
		ret = i2c_check_sensor(sensor_i2c_param[i].chip_addr,sensor_i2c_param[i].data_addr,
					sensor_i2c_param[i].data_len,sensor_i2c_param[i].check_data);
		if(ret == 0){
			sprintf(g_sensor_id,"%s",sensor_i2c_param[i].sensor_id);
			return 0;
		}else{
			i++;
		}
	}while(i < SENSOR_NUM_MAX);
	sprintf(g_sensor_id,"unknow");
	printf("[%s]not find!!\n",__func__);
	return 0;
}
//add end
