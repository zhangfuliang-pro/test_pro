...
#include "common_cmd_i2c_hongshi.h"
...
int set_i2c_dev(char *dev_num)
{
	...
	if(dev_num == NULL)
		return CMD_RET_USAGE;
	bus_no = simple_strtoul(dev_num, NULL, 10);
#if defined(CONFIG_SYS_I2C)
	if (bus_no >= CONFIG_SYS_NUM_I2C_BUSES) {
		printf("Invalid bus %d\n", bus_no);
		return -1;
...
int i2c_check_sensor(char *chip_addr,char *data_addr, char *len,char *sensor_id)
{
	...
	if ((chip_addr == NULL) || (data_addr == NULL) || (len == NULL))
		return CMD_RET_USAGE;
	// printf(">>>>[%s]%d\n",__func__,__LINE__);
	/*
	* I2C chip address
	*/
	chip = simple_strtoul(chip_addr, NULL, 16);
	if(sensor_i2c_probe(chip) <= 0){
		return -1;
	}
	// printf(">>>>[%s]%d\n",__func__,__LINE__);
	/*
	...
	* 2 bytes long.  Some day it might be 3 bytes long :-).
	*/
	addr = simple_strtoul(data_addr, NULL, 16);
	alen = get_alen(data_addr, DEFAULT_ADDR_LEN);
	if (alen > 3)
		return CMD_RET_USAGE;
	// printf(">>>>[%s]%d\n",__func__,__LINE__);
	/*
	...
	* Length is the number of objects, not number of bytes.
	*/
	length = simple_strtoul(len, NULL, 16);
	// printf(">>>>[%s]%d\n",__func__,__LINE__);
#ifdef CONFIG_DM_I2C
	ret = i2c_get_cur_bus_chip(chip, &dev);
	if (!ret && alen != -1)
