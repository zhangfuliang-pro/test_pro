...
#include <image.h>
#endif
#include "gpio_ctrl/mhal_gpio.h"
#include "gpio_ctrl/gpio.h"
...
//////////////////////////////////////////////////////////////////////
//add kjf for 2020/04/08
static int yw_led_ctrl(void)
{
	int PM_IRIN = 0;
	//step 1:init gpio
	MHal_GPIO_Init();
	...
	//step 2:set led gpio
	MHal_GPIO_Pad_Set(PAD_PM_IRIN);
	MHal_GPIO_Set_Output(PAD_PM_IRIN);
	MHal_GPIO_Set_Low(PAD_PM_IRIN);

	MHal_GPIO_Pad_Set(PAD_FUART_CTS);
	MHal_GPIO_Set_Output(PAD_FUART_CTS);
	MHal_GPIO_Set_High(PAD_FUART_CTS);

	MHal_GPIO_Pad_Set(PAD_FUART_RTS);
	MHal_GPIO_Set_Output(PAD_FUART_RTS);
	MHal_GPIO_Set_High(PAD_FUART_RTS);

	//step 3:get led gpio status
	PM_IRIN = MHal_GPIO_Pad_Level(PAD_PM_IRIN);
	printf("[%s][PM_IRIN:%d]\n",__func__,PM_IRIN);
	return 0;
}
//add end
